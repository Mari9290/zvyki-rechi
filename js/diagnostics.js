...
function result6(e) {
    ...
    if (points >= 8) {
        show6.textContent = 'Скорее всего, все ОК. Ведите дневник развития ребенка и наблюдайте динамику. Можно заниматься развитием речи дополнительно, для лучших результатов. Нажмите на кнопку, чтобы посмотреть варианты подписки на наши развивающие игры.'
    }
    if (points < 8) {
        show6.textContent = 'Этот результат означает, что есть над чем поработать. Лучше обратиться к логопеду для очной оценки ситуации и получению рекомендаций. Без паники! У нас есть игры для развития разных сторон речи - нажмите на кнопку, чтобы посмотреть варианты подписки.'
    }
